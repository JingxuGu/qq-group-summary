...
function renderMessageGroup(group, history) {
  ...
  card.querySelector('h3').textContent = group.name; card.querySelector('.message-group-count strong').textContent = group.message_count
...
$('email-form').addEventListener('submit', async (event) => { event.preventDefault(); const button = event.submitter; button.disabled = true; $('email-message').textContent = 'Sending confirmation…'; try { const response = await fetch('/api/v1/subscription', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email: $('subscription-email').value.trim(), daily_time: $('subscription-time').value }) }); const data = await response.json(); if (!response.ok) throw new Error(data.detail || 'Could not save subscription'); $('email-message').textContent = 'Subscription saved. Check your inbox for the confirmation email.' } catch (error) { $('email-message').textContent = error.message } finally { button.disabled = false } })
